...
srv = ServerProxy('http://paste.enzotools.org/xmlrpc/', allow_none=True)
...
def paste_id_from_url(url):
    regex = re.compile(r'^http://paste.enzotools.org/show/([^/]+)/?$')
    m = regex.match(url)
    if m is not None:
        return m.group(1)
...
# new paste or reply
    ...
    url = 'http://paste.enzotools.org/show/%s' % paste_id
